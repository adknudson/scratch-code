...
- jmp jumps to a new instruction relative to itself. The next instruction to 
  execute is found using the argument as an offset from the jmp instruction; for example, jmp +2 would skip the next instruction, jmp +1 would continue to the instruction immediately below it, and jmp -20 would cause the instruction 20 lines above to be executed next.
...
mutable struct Instructions
    ...
    Instructions(str) = begin
        input = [(Symbol(a), parse(Int, b)) for (a, b) in split.(readlines(str))]
        new(1, 0, length(input), Set{Int}(), input)
    end
...
after the program terminates, the accumulator contains the 
